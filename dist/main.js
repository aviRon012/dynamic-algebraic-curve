...
`;class l{x;y;constructor(t,i){this.x=t,this.y=i}set(t,i){return this.x=t,this.y=i,this}setVec(t){return this.x=t.x,this.y=t.y,this}add(t){return this.x+=t.x,this.y+=t.y,this}sub(t){return this.x-=t.x,this.y-=t.y,this}mult(t){return this.x*=t,this.y*=t,this}div(t){if(t!==0)this.x/=t,this.y/=t;return this}magSq(){return this.x*this.x+this.y*this.y}mag(){return Math.sqrt(this.x*this.x+this.y*this.y)}normalize(){let t=this.mag();if(t>0)this.div(t);return this}limit(t){let i=this.magSq();if(i>t*t)this.div(Math.sqrt(i)),this.mult(t);return this}static dist(t,i){let r=t.x-i.x,e=t.y-i.y;return Math.sqrt(r*r+e*e)}static distSq(t,i){let r=t.x-i.x,e=t.y-i.y;return r*r+e*e}static sub(t,i){return new l(t.x-i.x,t.y-i.y)}}var j=0.22,U=0.0055,V=0.06,N=2.5,q=6,B=[0.11764705882352941,0.1607843137254902,0.23137254901960785,1],L=[0.058823529411764705,0.09019607843137255,0.16470588235294117,1],D=[0.17647058823529413,0.8313725490196079,0.7490196078431373,1],K="#fbbf24";var J=0.3,W=25,H=80,f=50,m=1.5,Q=1,v=2;function X(){return{minDistance:100,maxSpeed:2.5,maxForce:0.15,scale:1,isPaused:!1,viewMode:0}}class w{pos;vel;acc;wanderTheta;_steer;_diff;_circleCenter;_displacement;constructor(t,i){this.pos=new l(t,i),this.vel=new l(Math.random()*2-1,Math.random()*2-1),this.acc=new l(0,0),this.wanderTheta=Math.random()*Math.PI*2,this._steer=new l(0,0),this._diff=new l(0,0),this._circleCenter=new l(0,0),this._displacement=new l(0,0)}applyForce(t){this.acc.add(t)}separate(t,i){let r=i.minDistance*i.minDistance;this._steer.set(0,0);let e=0;for(let o of t){let n=l.distSq(this.pos,o.pos);if(o!==this&&n<r&&n>0){let a=Math.sqrt(n);this._diff.setVec(this.pos).sub(o.pos),this._diff.normalize(),this._diff.div(a),this._steer.add(this._diff),e++}}if(e>0)this._steer.div(e),this._steer.normalize(),this._steer.mult(i.maxSpeed),this._steer.sub(this.vel),this._steer.limit(i.maxForce*m);return this._steer}wander(t){this.wanderTheta+=(Math.random()*2-1)*J,this._circleCenter.setVec(this.vel),this._circleCenter.normalize(),this._circleCenter.mult(H),this._displacement.set(0,-1),this._displacement.mult(W);let i=this.wanderTheta,r=this._displacement.x,e=this._displacement.y;this._displacement.x=r*Math.cos(i)-e*Math.sin(i),this._displacement.y=r*Math.sin(i)+e*Math.cos(i);let o=this._circleCenter.add(this._displacement);return o.limit(t.maxForce),o}edges(t,i,r){let e=this._diff;if(e.set(0,0),this.pos.x<f)e.x=r.maxSpeed;if(this.pos.x>t-f)e.x=-r.maxSpeed;if(this.pos.y<f)e.y=r.maxSpeed;if(this.pos.y>i-f)e.y=-r.maxSpeed;if(e.mag()>0)e.normalize(),e.mult(r.maxSpeed),e.sub(this.vel),e.limit(r.maxForce*v);return e}update(t,i,r,e){let o=this.separate(t,e),n=this.wander(e),a=this.edges(i,r,e);o.mult(m),n.mult(Q),a.mult(v),this.applyForce(o),this.applyForce(n),this.applyForce(a),this.vel.add(this.acc),this.vel.limit(e.maxSpeed),this.pos.add(this.vel),this.acc.mult(0)}draw(t){t.beginPath(),t.arc(this.pos.x,this.pos.y,q,0,Math.PI*2),t.fillStyle=K,t.fill(),t.strokeStyle="white",t.lineWidth=1,t.stroke()}}var Z=`// Solver Compute Shader
