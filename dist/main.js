...
`;class l{x;y;constructor(t,i){this.x=t,this.y=i}set(t,i){return this.x=t,this.y=i,this}setVec(t){return this.x=t.x,this.y=t.y,this}add(t){return this.x+=t.x,this.y+=t.y,this}sub(t){return this.x-=t.x,this.y-=t.y,this}mult(t){return this.x*=t,this.y*=t,this}div(t){if(t!==0)this.x/=t,this.y/=t;return this}magSq(){return this.x*this.x+this.y*this.y}mag(){return Math.sqrt(this.x*this.x+this.y*this.y)}normalize(){let t=this.mag();if(t>0)this.div(t);return this}limit(t){let i=this.magSq();if(i>t*t)this.div(Math.sqrt(i)),this.mult(t);return this}static dist(t,i){let r=t.x-i.x,e=t.y-i.y;return Math.sqrt(r*r+e*e)}static distSq(t,i){let r=t.x-i.x,e=t.y-i.y;return r*r+e*e}static sub(t,i){return new l(t.x-i.x,t.y-i.y)}}var U=0.22,V=0.0055,q=0.06,N=2.5,B=6,D=[0.11764705882352941,0.1607843137254902,0.23137254901960785,1],L=[0.058823529411764705,0.09019607843137255,0.16470588235294117,1],K=[0.17647058823529413,0.8313725490196079,0.7490196078431373,1],m="#fbbf24";var W=0.3,J=25,H=80,f=50,v=1.5,Q=1,w=2;function X(){return{minDistance:100,maxSpeed:2.5,maxForce:0.15,scale:1,isPaused:!1,viewMode:0}}class b{pos;vel;acc;wanderTheta;_steer;_diff;_circleCenter;_displacement;constructor(t,i){this.pos=new l(t,i),this.vel=new l(Math.random()*2-1,Math.random()*2-1),this.acc=new l(0,0),this.wanderTheta=Math.random()*Math.PI*2,this._steer=new l(0,0),this._diff=new l(0,0),this._circleCenter=new l(0,0),this._displacement=new l(0,0)}applyForce(t){this.acc.add(t)}separate(t,i){let r=i.minDistance*i.minDistance;this._steer.set(0,0);let e=0;for(let o of t){let n=l.distSq(this.pos,o.pos);if(o!==this&&n<r&&n>0){let a=Math.sqrt(n);this._diff.setVec(this.pos).sub(o.pos),this._diff.normalize(),this._diff.div(a),this._steer.add(this._diff),e++}}if(e>0)this._steer.div(e),this._steer.normalize(),this._steer.mult(i.maxSpeed),this._steer.sub(this.vel),this._steer.limit(i.maxForce*v);return this._steer}wander(t){this.wanderTheta+=(Math.random()*2-1)*W,this._circleCenter.setVec(this.vel),this._circleCenter.normalize(),this._circleCenter.mult(H),this._displacement.set(0,-1),this._displacement.mult(J);let i=this.wanderTheta,r=this._displacement.x,e=this._displacement.y;this._displacement.x=r*Math.cos(i)-e*Math.sin(i),this._displacement.y=r*Math.sin(i)+e*Math.cos(i);let o=this._circleCenter.add(this._displacement);return o.limit(t.maxForce),o}edges(t,i,r){let e=this._diff;if(e.set(0,0),this.pos.x<f)e.x=r.maxSpeed;if(this.pos.x>t-f)e.x=-r.maxSpeed;if(this.pos.y<f)e.y=r.maxSpeed;if(this.pos.y>i-f)e.y=-r.maxSpeed;if(e.mag()>0)e.normalize(),e.mult(r.maxSpeed),e.sub(this.vel),e.limit(r.maxForce*w);return e}update(t,i,r,e){let o=this.separate(t,e),n=this.wander(e),a=this.edges(i,r,e);o.mult(v),n.mult(Q),a.mult(w),this.applyForce(o),this.applyForce(n),this.applyForce(a),this.vel.add(this.acc),this.vel.limit(e.maxSpeed),this.pos.add(this.vel),this.acc.mult(0)}draw(t){t.beginPath(),t.arc(this.pos.x,this.pos.y,B,0,Math.PI*2),t.fillStyle=m,t.shadowColor=m,t.shadowBlur=10,t.fill(),t.shadowBlur=0,t.strokeStyle="white",t.lineWidth=1,t.stroke()}}var Z=`// Solver Compute Shader
...
`;class y{device;degree;pointCount;termCount;pipeline;bindGroup;particleBuffer;coeffsBuffer;prevCoeffsBuffer;paramsBuffer;matrixBuffer;isReady;constructor(t,i){this.device=t,this.degree=i,this.isReady=!1,this.termCount=(i+1)*(i+2)/2,this.pointCount=this.termCount-1,this.pipeline=null,this.bindGroup=null,this.particleBuffer=t.createBuffer({size:this.pointCount*2*4,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST}),this.coeffsBuffer=t.createBuffer({size:this.termCount*4,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_SRC|GPUBufferUsage.COPY_DST}),this.prevCoeffsBuffer=t.createBuffer({size:this.termCount*4,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST}),this.matrixBuffer=t.createBuffer({size:this.termCount*this.termCount*4,usage:GPUBufferUsage.STORAGE}),this.paramsBuffer=t.createBuffer({size:16,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),this.initPipeline()}async initPipeline(){let t=this.device.createShaderModule({code:Z});this.pipeline=await this.device.createComputePipelineAsync({layout:"auto",compute:{module:t,entryPoint:"main",constants:{degree:this.degree,pointCount:this.pointCount,termCount:this.termCount}}}),this.bindGroup=this.device.createBindGroup({layout:this.pipeline.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:this.particleBuffer}},{binding:1,resource:{buffer:this.coeffsBuffer}},{binding:2,resource:{buffer:this.paramsBuffer}},{binding:3,resource:{buffer:this.prevCoeffsBuffer}},{binding:4,resource:{buffer:this.matrixBuffer}}]}),this.isReady=!0}resize(t,i,r){let e=new Float32Array([r,t,i,0]);this.device.queue.writeBuffer(this.paramsBuffer,0,e)}solve(t){if(!this.pipeline||!this.bindGroup)return;let i=new Float32Array(this.pointCount*2);for(let o=0;o<this.pointCount;o++)i[o*2]=t[o].pos.x,i[o*2+1]=t[o].pos.y;this.device.queue.writeBuffer(this.particleBuffer,0,i);let r=this.device.createCommandEncoder(),e=r.beginComputePass();e.setPipeline(this.pipeline),e.setBindGroup(0,this.bindGroup),e.dispatchWorkgroups(1),e.end(),this.device.queue.submit([r.finish()])}getCoeffsBuffer(){return this.coeffsBuffer}dispose(){this.particleBuffer.destroy(),this.coeffsBuffer.destroy(),this.prevCoeffsBuffer.destroy(),this.paramsBuffer.destroy(),this.matrixBuffer.destroy()}}var k=`// src/shaders/curve.wgsl
...
`;var R=0,A=4,I=8,O=12,$=13,tt=14,it=64;class P{canvas;degree;adapter;device;context;pipeline;uniformBuffer;bindGroup;width;height;scale;uniformData;isReady;constructor(t,i){this.canvas=t,this.degree=i,this.adapter=null,this.device=null,this.context=null,this.pipeline=null,this.uniformBuffer=null,this.bindGroup=null,this.width=t.width,this.height=t.height,this.scale=1,this.uniformData=new Float32Array(it/4),this.setUniformColor(R,D),this.setUniformColor(A,L),this.setUniformColor(I,K),this.isReady=!1}setUniformColor(t,i){this.uniformData.set(i,t)}async init(t){if(!navigator.gpu)return"WebGPU is not supported.";if(t)this.device=t,this.adapter=null;else{if(this.adapter=await navigator.gpu.requestAdapter(),!this.adapter)return"No WebGPU adapter found.";this.device=await this.adapter.requestDevice()}if(this.context=this.canvas.getContext("webgpu"),!this.context)return"Failed to get WebGPU context";let i=navigator.gpu.getPreferredCanvasFormat();return this.context.configure({device:this.device,format:i,alphaMode:"premultiplied"}),this.uniformBuffer=this.device.createBuffer({size:this.uniformData.byteLength,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),await this.createPipeline(this.degree),this.isReady=!0,null}async createPipeline(t){if(!this.device)return;if(this.degree=t,!k)return;let i=this.device.createShaderModule({label:"Curve Shader",code:k}),r=navigator.gpu.getPreferredCanvasFormat();this.pipeline=this.device.createRenderPipeline({layout:"auto",vertex:{module:i,entryPoint:"vs_main"},fragment:{module:i,entryPoint:"fs_main",targets:[{format:r}],constants:{degree:t}},primitive:{topology:"triangle-strip"}})}resize(t,i,r){this.width=t,this.height=i,this.scale=r,this.uniformData[O]=t,this.uniformData[$]=i,this.uniformData[tt]=r}dispose(){if(this.uniformBuffer)this.uniformBuffer.destroy()}async draw(t){if(!this.isReady||!this.device||!this.pipeline||!this.context||!this.uniformBuffer)return;this.device.pushErrorScope("validation");try{this.device.queue.writeBuffer(this.uniformBuffer,0,this.uniformData),this.bindGroup=this.device.createBindGroup({layout:this.pipeline.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:this.uniformBuffer}},{binding:1,resource:{buffer:t}}]});let i=this.device.createCommandEncoder(),e={colorAttachments:[{view:this.context.getCurrentTexture().createView(),clearValue:{r:0,g:0,b:0,a:0},loadOp:"clear",storeOp:"store"}]},o=i.beginRenderPass(e);o.setPipeline(this.pipeline),o.setBindGroup(0,this.bindGroup),o.draw(4),o.end(),this.device.queue.submit([i.finish()]);let n=await this.device.popErrorScope();if(n)console.error("WebGPU Validation Error:",n.message),this.isReady=!1}catch(i){console.error("Render Error:",i),this.isReady=!1}}clear(){if(!this.isReady||!this.device||!this.context)return;let t=this.device.createCommandEncoder(),r={colorAttachments:[{view:this.context.getCurrentTexture().createView(),clearValue:{r:0,g:0,b:0,a:0},loadOp:"clear",storeOp:"store"}]};t.beginRenderPass(r).end(),this.device.queue.submit([t.finish()])}}var C=1,_=6;class M{glCanvas;uiCanvas;container;ctx;width;height;particles;solver;renderer;currentDegree;pointCount;params;draggedParticle;needsUpdate;onDegreeChange;onPauseChange;onViewModeChange;adapter;device;lastTime;frameCount;fps;onFpsUpdate;constructor(t,i,r){this.glCanvas=t,this.uiCanvas=i,this.container=r;let e=i.getContext("2d");if(!e)throw Error("Could not get 2D context");this.ctx=e,this.width=0,this.height=0,this.particles=[],this.solver=null,this.renderer=null,this.currentDegree=2,this.pointCount=0,this.params=X(),this.draggedParticle=null,this.needsUpdate=!0,this.adapter=null,this.device=null,this.onDegreeChange=null,this.onPauseChange=null,this.onViewModeChange=null,this.lastTime=performance.now(),this.frameCount=0,this.fps=0,this.onFpsUpdate=null,this.init()}showError(t){let i=document.createElement("div");if(i.style.position="absolute",i.style.top="50%",i.style.left="50%",i.style.transform="translate(-50%, -50%)",i.style.backgroundColor="rgba(220, 38, 38, 0.9)",i.style.color="white",i.style.padding="20px",i.style.borderRadius="8px",i.style.fontFamily="sans-serif",i.style.zIndex="1000",i.style.textAlign="center",i.innerHTML=`<h3>Initialization Error</h3><p>${t}</p>`,this.container)this.container.appendChild(i);else document.body.appendChild(i);let r=document.getElementById("ui-container");if(r)r.style.display="none"}async init(){if(!navigator.gpu){this.showError("WebGPU not supported.");return}if(this.adapter=await navigator.gpu.requestAdapter(),!this.adapter){this.showError("No WebGPU adapter found.");return}this.device=await this.adapter.requestDevice(),this.resize(),await this.setDegree(this.currentDegree),this.animate()}resize(){let t=this.container.clientWidth||window.innerWidth,i=this.container.clientHeight||window.innerHeight,r=window.devicePixelRatio||1;if(this.uiCanvas.width=t*r,this.uiCanvas.height=i*r,this.uiCanvas.style.width=`${t}px`,this.uiCanvas.style.height=`${i}px`,this.ctx.scale(r,r),this.glCanvas.width=t*r,this.glCanvas.height=i*r,this.width=t,this.height=i,this.updatePhysicsParams(),this.solver)this.solver.resize(this.width,this.height,this.params.scale);if(this.renderer)this.renderer.resize(this.glCanvas.width,this.glCanvas.height,this.params.scale*r);this.needsUpdate=!0}updatePhysicsParams(){if(!this.pointCount)return;let t=this.width*this.height/this.pointCount,i=Math.sqrt(t);this.params.minDistance=i*U,this.params.maxSpeed=i*V,this.params.maxForce=this.params.maxSpeed*q,this.params.scale=Math.min(this.width,this.height)/N}async setDegree(t){if(t<C||t>_)return;this.currentDegree=t;let i=(this.currentDegree+1)*(this.currentDegree+2)/2;if(this.pointCount=i-1,this.updatePhysicsParams(),this.spawnParticles(),this.device)await this.device.queue.onSubmittedWorkDone();if(this.renderer)this.renderer.dispose();if(this.solver)this.solver.dispose();if(!this.device)return;this.solver=new y(this.device,this.currentDegree),this.renderer=new P(this.glCanvas,this.currentDegree);let r=await this.renderer.init(this.device);if(r){this.showError(r);return}if(this.solver.resize(this.width,this.height,this.params.scale),this.renderer.resize(this.glCanvas.width,this.glCanvas.height,this.params.scale*(window.devicePixelRatio||1)),this.needsUpdate=!0,this.onDegreeChange)this.onDegreeChange(this.currentDegree)}spawnParticles(){this.particles=[];let t=this.params.minDistance;for(let i=0;i<this.pointCount;i++){let r=Math.random()*(this.width-t*2)+t,e=Math.random()*(this.height-t*2)+t;this.particles.push(new b(r,e))}this.needsUpdate=!0}togglePause(){if(this.params.isPaused=!this.params.isPaused,this.needsUpdate=!0,this.onPauseChange)this.onPauseChange(this.params.isPaused)}cycleViewMode(){if(this.params.viewMode=(this.params.viewMode+1)%3,this.needsUpdate=!0,this.onViewModeChange)this.onViewModeChange(this.params.viewMode)}handleInputStart(t,i){if(!this.params.isPaused)return!1;if(this.params.viewMode===1)return!1;let r=900;for(let e of this.particles){let o=e.pos.x-t,n=e.pos.y-i;if(o*o+n*n<r)return this.draggedParticle=e,!0}return!1}handleInputMove(t,i){if(!this.draggedParticle)return;if(!this.params.isPaused){this.draggedParticle=null;return}this.draggedParticle.pos.x=t,this.draggedParticle.pos.y=i,this.draggedParticle.vel.set(0,0),this.needsUpdate=!0}handleInputEnd(){this.draggedParticle=null}triggerUpdate(){this.needsUpdate=!0}animate(){let t=performance.now();if(this.frameCount++,t-this.lastTime>=1000){if(this.fps=this.frameCount,this.frameCount=0,this.lastTime=t,this.onFpsUpdate)this.onFpsUpdate(this.fps)}if(!this.params.isPaused||this.needsUpdate){if(!this.params.isPaused)for(let e of this.particles)e.update(this.particles,this.width,this.height,this.params);if(this.ctx.clearRect(0,0,this.width,this.height),this.params.viewMode===0||this.params.viewMode===2)for(let e of this.particles)e.draw(this.ctx);let r=!1;if(this.params.viewMode===0||this.params.viewMode===1){if(this.solver&&this.solver.isReady&&this.renderer){this.solver.solve(this.particles);let e=this.solver.getCoeffsBuffer();if(this.renderer.draw(e),this.renderer.isReady)r=!0}}else if(this.renderer){if(this.renderer.clear(),this.renderer.isReady)r=!0}if(this.params.viewMode===2||r)this.needsUpdate=!1}requestAnimationFrame(()=>this.animate())}}var et=`
    <style>${j}</style>
    <div id="ui-container">
        <div class="row">
            <span>Degree:</span>
